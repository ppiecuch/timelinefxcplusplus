...
public:
	...
	// Call this only once.
	void initialize(QGeometryData data, const QTransform &matrix, uint hint);
	// Call this only once.
...
template <typename T>
void QTriangulator<T>::initialize(QGeometryData data, const QTransform &matrix, uint hint)
{
	m_hint = hint;
	m_hint &= ~QVectorPath::CurvedShapeMask;
	m_vertices.resize(data.count());
	//m_indices.resize(count + 1);
	for (int i = 0; i < m_vertices.size(); ++i) {
		qreal x, y;
		QVector3D v=data.vertex(i);
		matrix.map(v.x(), v.y(), &x, &y);
		m_vertices.at(i).x = qRound(x * Q_FIXED_POINT_SCALE);
		m_vertices.at(i).y = qRound(y * Q_FIXED_POINT_SCALE);
	}
	for (int i = 0; i < (data.indexCount()>>1); ++i) {
		m_indices.push_back(T(data.indices().at(i*2)));
		if ((i+1==(data.indexCount()>>1)) || (data.indices().at(i*2+1)!=data.indices().at((i+1)*2))) {
			m_indices.push_back(T(data.indices().at(i*2+1)));
			m_indices.push_back(T(-1));
		}
		//m_indices[i] = i;
	}
	//m_indices.append(T(-1)); //Q_TRIANGULATE_END_OF_POLYGON
}
...
QTriangleSet qTriangulate(const QGeometryData &path,
						  uint hint, const QTransform &matrix)
{
	QTriangleSet triangleSet;
	//if (QGLExtensions::glExtensions() & QGLExtensions::ElementIndexUint) {
		QTriangulator<quint32> triangulator;
		triangulator.initialize(path, matrix,hint);
		QVertexSet<quint32> vertexSet = triangulator.triangulate();
		triangleSet.vertices = vertexSet.vertices;
		triangleSet.indices.setDataUint(vertexSet.indices);
	/*} else {
...
QPolylineSet qPolyline(const qreal *polygon,
					   int count, uint hint, const QTransform &matrix)
{
...
QPolylineSet qPolyline(const QGeometryData &path,
					   uint hint, const QTransform &matrix)
{
	QPolylineSet polyLineSet;
	//if (QGLExtensions::glExtensions() & QGLExtensions::ElementIndexUint) {
...
// Stripify triangle indicies list.
// --------------------------------
...
inline unsigned int hash(const Triangle &v)
{
    return (v.a * 31 ^ v.b) * 31 ^ v.c;
}
inline unsigned int hash(const Edge &v)
{
    return v.a * 31 ^ v.b;
}

void qStripifyProcess(QVector<quint32> &indices)
{
	...
	for(QVector<quint32>::const_iterator i = indices.begin() ; i != indices.end() ; ++i)
	{
		const quint32 a = *i;	++i;
		const quint32 b = *i;	++i;
		const quint32 c = *i;
	...
	qDebug() << "[qStripifyProcess] data structures filled";

	QTime timer;
	timer.start();

	indices.clear();
	QVector<quint32> stripE;
	QVector<quint32> stripS;
	while(!triangles.empty())
	{
        ...
        indices.resize(indices.size() + stripE.size() + stripS.size()); // insert new vectors
		qCopyBackward(stripS.rbegin(), stripS.rend(), indices.end()); // indices.insert(indices.end(), stripS.rbegin(), stripS.rend());
		qCopyBackward(stripE.begin(), stripE.end(), indices.end()); // indices.insert(indices.end(), stripE.begin(), stripE.end());
	}

	qDebug() << "[qStripifyProcess] done in " << timer.elapsed() << " ms";
}
